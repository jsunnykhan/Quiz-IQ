...
import 'package:quiz_iq/domain/usecase/Login/get_google_login.dart';


class MockLoginRepository extends Mock implements LoginRepo {}

void main() {
  GetGoogleLogin usecase;
  MockLoginRepository mockLoginRepository;

  setUp(() {
    mockLoginRepository = MockLoginRepository();
    usecase = GetGoogleLogin(mockLoginRepository);
  });
  ...
  test(
    '1. should  get google Login from Login Repository',
    () async {
      //arrange
      when(mockLoginRepository.getGoogleLogin())
          .thenAnswer((_) async => Right(tloginResult));
      //act
      final result = await usecase();
      //assert
      expect(result, Right(tloginResult));
      verify(mockLoginRepository.getGoogleLogin());
